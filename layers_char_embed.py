...
class CharEmbedding(nn.Module):
    def __init__(self, char_vectors, drop_prob, char_embed_size) -> None:
        ...
        self.char_embed_size = char_embed_size # whatever dimension we will use for the word vector, same we will use for the char vectors (output of the Char Embed layer) 

        self.char_embed = nn.Embedding.from_pretrained(char_vectors)

        self.cnn = nn.Sequential(nn.Conv1d(in_channels = char_vectors.size(1), out_channels =  self.char_embed_size, kernel_size = 3),# check dimensions passed here
                                nn.AdaptiveMaxPool1d(self.char_embed_size))
    ...
    def forward(self, char_idxs):
        ...
        assert(emb.shape == (batch_size, seq_len, self.char_embed_size))
